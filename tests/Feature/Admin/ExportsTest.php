...
it('queues a users xlsx export and registers it as processing', function (): void {
    Excel::fake();
    ...
    Excel::assertQueued($export->path, 'local', fn ($e): bool => $e instanceof UsersExport);
});

it('queues a terreiros xlsx export', function (): void {
    Excel::fake();

    Livewire::actingAs(userWithRole('admin'))->test(TerreirosIndex::class)->call('export');

    $export = Export::query()->first();
    Excel::assertQueued($export->path, 'local', fn ($e): bool => $e instanceof TerreirosExport);
});

it('marks an export ready and notifies Telegram on finalize', function (): void {
    ...
    Http::assertSent(fn ($r): bool => str_contains((string) $r->url(), '/sendMessage'));
});

it('downloads the file and removes it from the server', function (): void {
    Storage::fake('local');
    $user = userWithRole('super-admin');
...
it('forbids downloading another users export', function (): void {
    Storage::fake('local');
