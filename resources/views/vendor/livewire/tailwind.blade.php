                    ...
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <button type="button" wire:click="gotoPage({{ $page }})"
                                    class="hidden h-9 min-w-9 items-center justify-center rounded-lg border border-slate-200 px-3 text-sm text-slate-600 transition hover:bg-slate-50 hover:text-slate-900 sm:inline-flex">{{ $page }}</button>
                        @endif
                    @endforeach
                @endif
            @endforeach
