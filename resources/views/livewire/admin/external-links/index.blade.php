                        ...
                        <tr class="hover:bg-slate-50" wire:key="link-{{ $link->id }}">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2 text-xs">
                                    <button wire:click="edit({{ $link->id }})" class="rounded px-2 py-1 text-violet-600 hover:bg-violet-50">Editar</button>
                                    <button wire:click="toggleStatus({{ $link->id }})" class="rounded px-2 py-1 text-slate-600 hover:bg-slate-100">
                                        {{ $link->status === \App\Enum\Status::ACTIVE ? 'Inativar' : 'Ativar' }}
                                    </button>
                                    <button wire:click="delete({{ $link->id }})" wire:confirm="Excluir este link?" class="rounded px-2 py-1 text-rose-600 hover:bg-rose-50">Excluir</button>
                                </div>
                            </td>
                        </tr>
                    @empty
