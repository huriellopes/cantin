<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800">{{ $form->editingId ? __('crud_posts.modal_edit_title') : __('crud_posts.modal_new_title') }}</h2>
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <x-admin.input label="{{ __('crud_posts.field_title') }}" name="form.titleField" wire:model="form.titleField" />
                <x-admin.input label="{{ __('crud_posts.field_slug') }}" name="form.slug" wire:model="form.slug" />

                <div class="space-y-1">
                    <label for="category_id" class="block text-sm font-medium text-slate-700">{{ __('crud_posts.field_category') }}</label>
                    <select id="category_id" wire:model="form.category_id" class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-violet-500 focus:ring-violet-500">
                        <option value="">{{ __('crud_posts.select_placeholder') }}</option>
                        @foreach ($categories as $id => $label)
                            <option value="{{ $id }}">{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('form.category_id') <p class="text-xs text-rose-600">{{ $message }}</p> @enderror
                </div>

                <x-admin.input label="{{ __('crud_posts.field_published_at') }}" name="form.published_at" type="date" wire:model="form.published_at" />
            </div>

            <div class="mt-4 space-y-1">
                <label for="image" class="block text-sm font-medium text-slate-700">{{ __('crud_posts.field_image') }}</label>
                @if ($currentImage && ! $form->image)
        ...
        <div class="space-y-1">
            ...
            <x-admin.quill-editor wire:model="form.content" :initial="$form->content" />
            @error('form.content') <p class="text-xs text-rose-600">{{ $message }}</p> @enderror
        </div>
